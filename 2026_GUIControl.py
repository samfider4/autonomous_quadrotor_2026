...
class DroneController:
    ...
    def pid_control(self, cur, tgt, yaw, dt):
        err = tgt - cur
        ...
        pd, rd = np.rad2deg(pitch), np.rad2deg(roll)
    ...
    def land(self):
        if self.init_pos is None:
            print("Init not ready")
            return
        self.target = self.init_pos.copy()
    ...
    def _control_loop(self):
        ...
        while self.control:
            ...
            safe_tgt = self._dodge_box(self.target.copy())
            pd, rd, th = self.pid_control(cur, safe_tgt, yaw, dt)
            self._cf.commander.send_setpoint(rd, pd, 0, th)
